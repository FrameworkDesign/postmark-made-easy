...
namespace Weareframework\PostmarkMadeEasy\Library\Env;
...
class EnvEditor
{
    ...
    {
        $backupPath      = Str::finish(config('statamic.postmark-made-easy.backupPath'), '/');
        $env             = config('statamic.postmark-made-easy.pathToEnv');
        $filePermissions = config('statamic.postmark-made-easy.filePermissions');
     ...
     *
     * @return mixed
     * @throws PostmarkMadeEasyException
     */
    public function getValue($key)
    {
        if ($this->keyExists($key)) {
            ...
            throw new PostmarkMadeEasyException('Requested value not available', 0);
        }
    }
    ...
    public function setAutoBackup($onOff)
    {
        if (is_bool($onOff)) {
            ...
            throw new PostmarkMadeEasyException('Autobackup wrong value', 0);
        }
    }
    ...
    protected function getFile($timestamp)
    {
        ...
        if (file_exists($file)) {
            ...
            throw new PostmarkMadeEasyException('requested backup not found', 0);
        }
    ...
    public function getBackupVersions()
    {
        ...
        throw new PostmarkMadeEasyException('no backups available', 0);
    }
    ...
    public function deleteBackup($timestamp)
    {
        ...
        if (file_exists($file)) {
            ...
            throw new PostmarkMadeEasyException('backup not deletable', 0);
        }
    }
    ...
    {
        $stringPre      = file_get_contents($file);
        $string      = preg_split('/\n+/', $stringPre);
        $returnArray = array();

//        dd('envToArray', $string, $stringPre);
    ...
    public function changeEnv($data = array())
    {
        ...
        throw new PostmarkMadeEasyException('array needed', 0);
    }
    ...
    public function deleteData($data = array())
    {
        foreach ($data as $key => $value) {
            if (!is_numeric($key)) {
                throw new PostmarkMadeEasyException("postmark-made-easy::class.numeric_array_needed", 0);
            }
        }
